...
class Trainer(Base):
    ...
    def get_optimizer(self, model):
        ...
        optim_params = [{"params": normal_param, "lr": self.cfg.train.lr}]
    ...
    def _make_batch_generator(self):
        ...
        for humandata_dataset in self.cfg.data.trainset_humandata:
            trainset_humandata_loader.append(
                dynamic_import(f"datasets.{humandata_dataset}", humandata_dataset)(
                    transforms.ToTensor(), "train", self.cfg
                )
            )
        ...
        if data_strategy == "concat":
            ...
            trainset_loader = MultipleDatasets(
                trainset_humandata_loader, make_same_len=False, verbose=True
            )
        ...
        self.itr_per_epoch = math.ceil(
            ...
            / self.cfg.train.train_batch_size
        )
        ...
        if self.distributed:
            ...
            self.batch_generator = DataLoader(
                ...
                shuffle=False,
                num_workers=self.cfg.train.num_thread,
                sampler=sampler_train,
                pin_memory=True,
                persistent_workers=True if self.cfg.train.num_thread > 0 else False,
                drop_last=True,
            )
        else:
            self.batch_generator = DataLoader(
...
class Tester(Base):
    ...
    def _make_batch_generator(self):
        ...
        testset_loader = dynamic_import(
            f"datasets.{self.cfg.data.testset}", self.cfg.data.testset
        )(transforms.ToTensor(), "test", self.cfg)
        batch_generator = DataLoader(
            dataset=testset_loader,
            batch_size=self.cfg.test.test_batch_size,
            shuffle=False,
            num_workers=1,
            pin_memory=True,
        )
    ...
    def _make_model(self):
        self.logger.info(
            "Load checkpoint from {}".format(self.cfg.model.pretrained_model_path)
        )
        ...
        ckpt = torch.load(
            self.cfg.model.pretrained_model_path, map_location=torch.device("cpu")
        )
