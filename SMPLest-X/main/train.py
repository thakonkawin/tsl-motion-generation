...
def main():
    ...
    config_path = osp.join(
        "./configs", args.config
    )  # TODO: move config folder outsied main
    ...
    new_config = {
        ...
        "log": {
            ...
            "model_dir": osp.join(
                root_dir, "outputs", f"{args.exp_name}_{time_str}", "model_dump"
            ),
            "log_dir": osp.join(
                root_dir, "outputs", f"{args.exp_name}_{time_str}", "log"
            ),
            "result_dir": osp.join(
                root_dir, "outputs", f"{args.exp_name}_{time_str}", "result"
            ),
        },
    }
    ...
    trainer.logger_info(
        f"Using {cfg.train.num_gpus} GPUs with bs={cfg.train.train_batch_size} per GPU."
    )
    ...
    for epoch in range(trainer.start_epoch, cfg.train.end_epoch):
        ...
        for itr, (inputs, targets, meta_info) in enumerate(trainer.batch_generator):
            ...
            if (itr + 1) % cfg.train.print_iters == 0:
                ...
                screen = [
                    "Epoch %d/%d itr %d/%d:"
                    % (epoch, cfg.train.end_epoch, itr, trainer.itr_per_epoch),
                    "lr: %g" % (trainer.get_lr()),
        ...
        if is_main_process() and (
            epoch % save_epoch == 0 or epoch == cfg.train.end_epoch - 1
        ):
            trainer.save_model(
                {
                    "epoch": epoch,
            ...
            if previous_saved_epoch is not None and remove_previous:
                to_remove = osp.join(
                    cfg.log.model_dir, f"snapshot_{str(previous_saved_epoch)}.pth.tar"
                )
                os.remove(to_remove)
