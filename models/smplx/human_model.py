...
from extern_files.smplx import smplx
import pickle
...
class SMPLX(object):
    def __init__(self):
        ...
        self.layer = {'neutral': smplx.create("assets/", model_type = 'SMPLX', gender='NEUTRAL_2020', use_pca=False, use_face_contour=True, **self.layer_arg),
                        # 'male': smplx.create("assets/", model_type= 'smplx', gender='MALE', use_pca=False, use_face_contour=True, **self.layer_arg),
                        # 'female': smplx.create("assets/", model_type= 'smplx', gender='FEMALE', use_pca=False, use_face_contour=True, **self.layer_arg)
                        }
        self.vertex_num = 10475
        self.face = self.layer['neutral'].faces
        self.shape_param_dim = 10
        self.expr_code_dim = 10
        with open(osp.join("assets/", 'SMPLX', 'SMPLX_to_J14.pkl'), 'rb') as f:
            self.j14_regressor = pickle.load(f, encoding='latin1')
        with open(osp.join("assets/", 'SMPLX', 'MANO_SMPLX_vertex_ids.pkl'), 'rb') as f:
            self.hand_vertex_idx = pickle.load(f, encoding='latin1')
        self.face_vertex_idx = np.load(osp.join("assets/", 'SMPLX', 'SMPL-X__FLAME_vertex_ids.npy'))
...
class SMPL(object):
    def __init__(self):
        ...
        self.layer = {'neutral': smplx.create("assets/", 'SMPL', gender='NEUTRAL', **self.layer_arg),
                    #    'male': smplx.create("assets/", 'smpl', gender='MALE', **self.layer_arg), 
                    #    'female': smplx.create("assets/", 'smpl', gender='FEMALE', **self.layer_arg)
                    }
