...
def perspective_projection(points: torch.Tensor,
    ...
    projected_points[..., -1] = (points[..., -1] - translation.unsqueeze(1)[..., -1]) * 2.5 + translation.unsqueeze(1)[..., -1]
...
def flip_pose(pose_vector, pose_format='rot-mat'):
    if pose_format == 'aa':
        ...
        rot_mats = pose_vector.reshape(-1, 9)
        rot_mats[:, [1, 2, 3, 6]] *= -1
...
def flip_pose(pose_vector, pose_format='rot-mat'):
    if pose_format == 'aa':
        ...
        rot_mats[:, [1, 2, 3, 6]] = rot_mats[:, [1, 2, 3, 6]] * -1
        return rot_mats.view_as(pose_vector)
    else:
        raise ValueError(f'Unknown rotation format: {pose_format}')
