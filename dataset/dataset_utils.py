...
def do_augmentation() -> Tuple:
    ...
    rot = np.clip(np.random.randn(), -2.0,
                  2.0) * ROT_FACTOR if random.random() <= ROT_AUG_RATE  else 0
    do_flip = DO_FLIP and random.random() <= FLIP_AUG_RATE # aug_config.FLIP_AUG_RATE
    do_extreme_crop = random.random() <= EXTREME_CROP_AUG_RATE 
    extreme_crop_lvl = EXTREME_CROP_AUG_LEVEL  # aug_config.get('EXTREME_CROP_AUG_LEVEL', 0)
    # extreme_crop_lvl = 0
    c_up = 1.0 + COLOR_SCALE
    c_low = 1.0 - COLOR_SCALE
...
def generate_image_patch_cv2(img: np.array, c_x: float, c_y: float,
    ...
    if do_flip:
        img = img[:, ::-1, :]
        c_x = img_width - c_x - 1
    org_trans = gen_trans_from_patch_cv(c_x, c_y, bb_width, bb_height, bb_width, bb_height, 1.0, 0)
...
#                              56, 63, 64, 65, 60, 61, 62]            # 69, 70, 71, 66, 67, 68          #  丢掉  
...
def fliplr_params(smpl_params: Dict) -> Tuple[Dict, Dict]:
    ...
    if  'left_hand_pose' in smpl_params:
        ...
        left_hand_pose = smpl_params['right_hand_pose'].copy()
        right_hand_pose = smpl_params['left_hand_pose'].copy()
...
def get_example(img_path: str, center_x: float, center_y: float,
    ...
    if do_augment:
        scale, rot, do_flip, do_extreme_crop, extreme_crop_lvl, color_scale, tx, ty = do_augmentation()
        if smpl_params['has_flame']:
            do_flip = False 
    else:
    ...
    for n_c in range(min(img_channels, 3)):
        ...
        if mean is not None and std is not None: # 后处理 加均值 除以标准差
    ...
    keypoints_2d[:, :-1] = keypoints_2d[:, :-1] / patch_width  # 扩展到 [0,1]范围

    if not return_trans:
        return img_patch, keypoints_2d, keypoints_3d, smpl_params, img_size
    else:
        return img_patch, keypoints_2d, keypoints_3d, smpl_params, img_size, trans
